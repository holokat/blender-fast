...
For construction, inspect [shape programs](shape-programs.md) and [scene recipes](scene-recipes.md) before writing a new factory. Compose reusable definitions and apply revisions by stable assembly ID. Catalog inspection and composition work without a custom model. Keep unsupported design work in focused definitions or batched Python, and preserve the brief.

For preview and refinement, use `interactive` (Eevee 4, 800 × 550), `optNEW` (Cycles 8, 1600 × 1100), then `opt1` (Cycles 64, 1600 × 1100) when appropriate. These are explicit quality choices. Keep one Blender process alive through related renders, reuse an existing preview server, and show freshly rendered Blender images for lighting decisions. A browser mesh viewer can use different lighting. The user's required final settings take precedence.
...
3. **Build in useful batches.** Use the shape-program or recipe compiler when it fits, passing the owned scene for revisions and omitting it for a new isolated scene. Keep stable IDs and inspect its rebuilt/transformed/removed report. For custom Python, load the task's trusted builder once, then send short stage or range commands. Start with up to 64 inexpensive objects per request and adapt to stage cost and responsiveness. Keep all `bpy` mutations on Blender's main thread. Reuse mesh and material data when it preserves the desired object behavior. Return created counts or IDs from each acknowledged stage.
